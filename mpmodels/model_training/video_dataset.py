...
from mpmodels.core.video_utils import label_id_convert
...
class VideoRecord(object):
    ...
    def __init__(self, row, root_datapath, id_to_label_dict=None):
        self._data = row
        if id_to_label_dict is None:
            ...
            self._path = os.path.join(root_datapath, id_to_label_dict[curr_id], row[0])
...
class VideoFrameDataset(data.Dataset):
    ...
    Args:
        ...
        transform_per_frame: If True, then applies the transform to each PIL image,
                             e.g. when you use ToTensor() instead of ImglistToTensor.
                             This is useful if you want to directly apply ImageNet
                             transforms without modification, like for the pretrained
                             frozen encoders.
    ...
    def __init__(
        ...
        specificfile_path: str = None,
        path_label_include=None,
        num_segments: int = 3,
        frames_per_segment: int = 1,
        imagefile_template: str = "img_{:05d}.jpg",
        transform=None,
        transform_per_frame: bool = False,
        ...
        self.specificfile_path = specificfile_path
        self.id_to_label_dict = None
        if path_label_include is not None:
            _, id_to_label_dict = label_id_convert(path_label_include)
        ...
        print(
            f"Total number of videos after filtering: {len(self.video_list)}, originally {len(self.initial_video_list)}"
        )
    ...
    def _parse_annotationfile(self):
        if self.specificfile_path is not None:
            self.initial_video_list = [
                VideoRecord(
                    row=x.strip().split(),
                    root_datapath=self.root_path,
                    id_to_label_dict=self.id_to_label_dict,
                )
                for x in open(self.annotationfile_path)
                if x.strip().split()[0] == self.specificfile_path
            ]
            assert len(self.initial_video_list) == 1
        else:
            self.initial_video_list = [
                VideoRecord(
                    row=x.strip().split(),
    ...
    def _sanity_check_samples(self):
        if self.specificfile_path is not None:
            ...
            if self.num_segments is None:
                self.num_segments = self.initial_video_list[0].num_frames
            if self.frames_per_segment is None:
                self.frames_per_segment = 1

        assert self.num_segments is not None
        assert self.frames_per_segment is not None
        self.video_list = []
        for record in self.initial_video_list:
            if record.num_frames <= 0 or record.start_frame == record.end_frame:
                if self.verbose:
                    print(
                        f"\nIgnoring video {record.path}, it seems to have zero RGB frames on disk!\n"
                    )
                continue
                ...
                if self.verbose:
                    print(
                        f"\nIgnoring video {record.path}, it has {record.num_frames} frames "
    ...
    def _get(
        ...
        if self.verbose:
            print("Frame indices", frame_start_indices)
        images = list()
