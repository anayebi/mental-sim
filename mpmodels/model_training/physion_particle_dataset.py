# adapted from: https://github.com/htung0101/Physion-particles/blob/5ac002aac1781b7cde2a353e1e368d3e3bea1030/data.py
import os
import torch
import time
import random
...
def load_data(data_names, path, mode=None):
    ...
    if mode == "avg":
        ...
        for itemid in range(nitems):
            outputs.append(
                np.mean(np.stack([data[itemid] for data in multiple_data], 0), 0)
            )
        return outputs
    else:
        raise ValueError
...
def load_data_dominoes(
    data_names,
    path,
    static_data_info,
    load_data_names=["obj_positions", "obj_rotations"],
):
    """
                ...
                if flex_engine:
                    ...
                    if is_subsample:
                        ...
                        for obj_id in range(n_objects):
                            st, ed = (
                                instance_idx_old[obj_id],
                                instance_idx_old[obj_id + 1],
                            )
                    ...
                    if "ok_obj_id" in static_data_info:
                        ...
                        if (
                            "bad_lamp" in static_data_info
                            and obj_positions.shape[0]
                            != static_data_info["before_fix_n_objects"]
                        ):
                            print(
                                "good trial with bad lamp",
                                static_data_info["trial_dir"],
                            )
                            import ipdb
                        ...
                        for idx, obj_id in enumerate(static_data_info["ok_obj_id"]):
                            ...
                            transformed_pts = np.matmul(
                                rot, obj_points[idx].T
                            ).T + np.expand_dims(trans, axis=0)
        ...
        for item in data_names:
            if item == "positions":
                ...
                if t == 0:
                    ...
                    current_data.append(
                        (positions_over_T[t] - positions_over_T[t - 1]) / dt
                    )
            else:
...
def remove_large_obstacles(phases_dict):
    if phases_dict["n_particles"] > 3000:
        ...
        for obj_id in range(n_objects):
            ...
            obj_npts = (
                phases_dict["instance_idx"][obj_id + 1]
                - phases_dict["instance_idx"][obj_id]
            )
            # print(obj_id, obj_npts, phases_dict["instance"][obj_id])
            if (
                phases_dict["instance"][obj_id] not in critical_objects
                and obj_npts > 3000
            ):
                is_removed = True
                continue
...
def subsample_particles_on_large_objects(phases_dict, limit=3000):
    ...
    for obj_id in range(n_objects):
        ...
        if obj_npts > limit and obj_names[obj_id] != b"cloth_square":
            ...
            is_subsample = True
            phases_dict["clusters"][obj_id][0][0] = phases_dict["clusters"][obj_id][0][
                0
            ][selected_idxs]
        else:
    ...
    phases_dict["instance_idx_before_subsample"] = copy.deepcopy(
...
def correct_bad_chair(phases_dict):
    ...
    if len(phases_dict["instance_idx"]) - 1 != phases_dict["n_objects"]:
        ...
        if (
            "drop" in phases_dict["trial_dir"]
            and "train/50" in phases_dict["trial_dir"]
        ):
...
def make_hierarchy(
    ...
    attr, relations, relations_types, node_r_idx, node_s_idx, pstep = (
        [attr],
        [],
        [],
        [],
    ...
    for i in range(n_root_level):
        ...
        for j in range(root_num):
            ...
            rels += [
                np.stack([roots, desendants, np.zeros(desendants.shape[0])], axis=1)
            ]
            rels_rev += [
                np.stack([desendants, roots, np.zeros(desendants.shape[0])], axis=1)
            ]
            if verbose:
                print(
                    np.max(
                        np.sqrt(
                            np.sum(np.square(pos[st + desendants, :3] - centers[j]), 1)
                        )
                    )
                )
        ...
        if verbose:
            print(
                np.max(
                    np.sqrt(np.sum(np.square(centers[siblings, :3] - centers[j]), 1))
                )
            )
        ...
        for j in range(root_num):
            ...
            if var:
                ...
                velocities += [
                    torch.mean(velocities[0][st:ed, :][ids], 0, keepdim=True)
                ]
            else:
...
def prepare_input(data, stat, args, phases_dict, verbose=0, var=False):
    ...
    if args.env == "TDWdominoes" and args.floor_cheat:
        ...
        floor = np.ones(nodes.shape[0], dtype=np.int) * (
            n_particles + 0
        )  # 0 for idx starting from zero
            ...
            if args.env == "TDWdominoes":
                if phases_dict["material"][i] == "rigid":
                    ...
                    anchors = np.concatenate(
                        (np.arange(st), np.arange(ed, n_particles))
                    )
                elif (
                    phases_dict["material"][i] == "fluid"
                    or phases_dict["material"][i] == "cloth"
                ):
                    attr[st:ed, 1] = 1
                    queries = np.arange(st, ed)
                    anchors = np.arange(n_particles)
...
class PhysicsFleXDataset(Dataset):
    def __init__(self, root_path, args, phase, verbose=0, scenarios=PHYSION_SCENARIOS):
        self.args = args
        self.phase = phase
        ...
        self.data_dir = [
            os.path.join(root_path, scenario) for scenario in self.scenarios
        ]
        ...
        if self.args.n_rollout == None:
            ...
            for ddir in self.data_dir:
                file = open(os.path.join(ddir, f"{phase}.txt"), "r")
                ...
                self.all_trials += [
                    os.path.join(ddir_root, trial_name) for trial_name in trial_names
                ]
    ...
    def augment_worldcoord(self, list_of_data):
        ...
        rot = np.array(
            [
                [np.cos(math.radians(azimuth)), 0, -np.sin(math.radians(azimuth))],
                [0, 1, 0],
        ...
        for data in list_of_data:
            ...
            transformed_positions = np.matmul(
                RT,
                np.concatenate([positions, np.ones((positions.shape[0], 1))], axis=1).T,
            ).T[:, :3]
            transformed_velocities = np.matmul(
                R,
                np.concatenate(
                    [velocities, np.ones((velocities.shape[0], 1))], axis=1
                ).T,
            ).T[:, :3]
    ...
    def __getitem__(self, idx):
        ...
        data_nxt_path = os.path.join(
            data_dir, trial_fullname, str(int(idx_timestep + self.training_fpt)) + ".h5"
        )
        current_oldest = 0
        ...
        data_prev_path = os.path.join(
            data_dir,
            trial_fullname,
            str(max(0, int(idx_timestep - self.training_fpt))) + ".h5",
        )
        data_prev = load_data_dominoes(self.data_names, data_prev_path, phases_dict)
        _, data, data_nxt = recalculate_velocities(
            [data_prev, data, data_nxt], self.dt, self.data_names
        )
        ...
        path = os.path.join(
            data_dir,
            trial_fullname,
            str(max(0, int(idx_timestep - (1) * self.training_fpt))) + ".h5",
        )
        ...
        for i in range(self.args.n_his):
            path = os.path.join(
                data_dir,
                trial_fullname,
                str(max(0, int(idx_timestep - (i + 1 + 1) * self.training_fpt)))
                + ".h5",
            )
            if self.args.env == "TDWdominoes":
                data_prev = load_data_dominoes(self.data_names, path, phases_dict)
                _, data_his = recalculate_velocities(
                    [data_prev, data_cur], self.dt, self.data_names
                )
                data_cur = data_prev
            else:
        ...
        if self.augment_coord or self.training_fpt > 1:
            ...
            path = os.path.join(
                data_dir,
                trial_fullname,
                str(max(0, int(idx_timestep - current_oldest - self.training_fpt)))
                + ".h5",
            )
        ...
        if self.args.noise_std > 0:
            ...
            vel_sequence_noise = np.random.normal(
                0, self.args.noise_std / (n_velsteps ** 0.5), size=(n_velsteps, vel_dim)
            )
        ...
        attr, state, relations, n_particles, n_shapes, instance_idx = prepare_input(
            data, self.stat, self.args, phases_dict, self.verbose
        )
