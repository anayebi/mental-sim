...
__all__ = [
    "FitVid",
    "fitvid_physion_64x64",
    "fitvid_k700_64x64",
    "fitvid_k700_aug_64x64",
    "fitvid_k700_128x128",
    "fitvid_k700_224x224",
    "fitvid_bridge_64x64",
    "fitvid_bridge_aug_64x64",
    "fitvid_physion_roll_aug_64x64",
    "fitvid_physion_aug_64x64",
    "fitvid_physion_aug_64x64_2",
    "fitvid_ctxt7_physion_aug_64x64",
    "fitvid_ctxt7_physion_aug_frames_64x64",
    "fitvid_ctxt7_physion_64x64",
]
...
class SEBlock(nn.Module):
    ...
    def __init__(
        self, input_size, hidden_size, act=F.relu, dim=(-2, -1), dtype=torch.float32
    ):
        super().__init__()
...
class EncoderBlock(nn.Module):
    def __init__(
        ...
        self.shortcut = nn.Conv2d(
            self.input_size, self.num_channels, 1, stride=self.stride, bias=False
        )
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            self.input_size, self.num_channels * self.expand, 1, bias=False
        )
        self.conv2 = nn.Conv2d(
        ...
        self.conv3 = nn.Conv2d(
            self.num_channels * self.expand, self.num_channels, 1, bias=False
        )
        self.shortcut = nn.Conv2d(self.input_size, self.num_channels, 1, bias=False)
...
class NvaeDecoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.linear = nn.Linear(self.input_size, np.prod(self.first_block_shape))
    ...
    def _build_blocks(self, decoder_block, up_block):
        ...
        self.output_conv = nn.Conv2d(
            out_ch, self.output_size, 3, bias=False, padding="same"
        )
...
class MultiGaussianLSTM(nn.Module):
    ...
    def init_states(self, batch_size, device):
        ...
        for i in range(self.num_layers):
            states[i] = (
                torch.zeros((batch_size, self.layers[i].hidden_size))
                .to(self.dtype)
                .to(device),
                torch.zeros((batch_size, self.layers[i].hidden_size))
                .to(self.dtype)
...
class FitVid(nn.Module):
    ...
    def _broadcast_context_frame_skips(self, skips, frame, num_times):
        ...
        skips = {k: _broadcast(v[:, frame]) for k, v in skips.items()}
        return skips
    ...
    def forward(self, video, actions=None):
        ...
        if self.verbose:
            print(f"Curr n past in fitvid: {self.n_past}")
        ...
        # we don't want to overwrite this, so we copy it instead
        self.layers["encoder"] = hidden.detach().clone()
        ...
        kld = torch.tensor(0.0, dtype=self.dtype).to(video.device)
        means, logvars = [], []
        if self.training:
            h_preds_pre = []
            h_preds = []
            ...
            h_preds_pre = torch.stack(h_preds_pre, axis=1)
            ...
            for t in range(1, self.T):
                ...
                if t > self.n_past:
                    ...
                    self.layers["encoder"][:, t] = h.detach().clone()
        ...
        self.layers["dynamics"] = h_preds.detach().clone()
        self.layers["decoder"] = preds.detach().clone()
...
def fitvid_context5(n_past=5, **kwargs):
    """Fitvid with 5 context frames"""
    assert ("n_past" not in kwargs.keys()) and (
        "n_past" not in FITVID_DEFAULT_CFG.keys()
    )
    for k in kwargs.keys():
        assert k not in FITVID_DEFAULT_CFG.keys()
    model = FitVid(n_past=n_past, **FITVID_DEFAULT_CFG, **kwargs)
    return model


def fitvid_context7(n_past=7, **kwargs):
    """Fitvid with 7 context frames"""
    assert ("n_past" not in kwargs.keys()) and (
        "n_past" not in FITVID_DEFAULT_CFG.keys()
    )
    for k in kwargs.keys():
...
def fitvid_physion_64x64(**kwargs):
    return fitvid_context5(**kwargs)


def fitvid_k700_64x64(**kwargs):
    return fitvid_context5(**kwargs)


def fitvid_k700_aug_64x64(**kwargs):
    return fitvid_context5(**kwargs)


def fitvid_k700_128x128(**kwargs):
    return fitvid_context5(**kwargs)


def fitvid_k700_224x224(**kwargs):
    return fitvid_context5(**kwargs)


def fitvid_bridge_64x64(**kwargs):
    return fitvid_context5(**kwargs)


def fitvid_bridge_aug_64x64(**kwargs):
    return fitvid_context5(**kwargs)


def fitvid_physion_roll_aug_64x64(**kwargs):
    return fitvid_context5(**kwargs)


def fitvid_physion_aug_64x64(**kwargs):
    return fitvid_context5(**kwargs)


def fitvid_physion_aug_64x64_2(**kwargs):
    return fitvid_context5(**kwargs)


def fitvid_ctxt7_physion_aug_64x64(**kwargs):
    return fitvid_context7(**kwargs)


def fitvid_ctxt7_physion_aug_frames_64x64(**kwargs):
    return fitvid_context7(**kwargs)


def fitvid_ctxt7_physion_64x64(**kwargs):
    return fitvid_context7(**kwargs)
